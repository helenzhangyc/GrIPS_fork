...
label_probs, calibrated_label_probs, raw_acc_count, raw_cal_acc_count, answer_list, index_list, _ = run(
    ...
    function=custom_instruction_prompt,
    split='test',
    modified={'Definition': base_candidate},
    task_labels=task_labels,
    if_calibrate=False
)
...
raw_acc = balanced_accuracy_score(answer_list, preds)
macro_f1 = f1_score(answer_list, preds, average='macro')
precision = precision_score(answer_list, preds, average='macro')
recall = recall_score(answer_list, preds, average='macro')

# Print results
print('Accuracy after search:\t', raw_acc)
print('Macro F1 after search:\t', macro_f1)
print('Precision after search:\t', precision)
print('Recall after search:\t', recall)
print('Instruction after search:\t', base_candidate)
print('Edit Operations:\t', operations_tracker)

# Save results to the metadata file
meta_file.write(f'Accuracy after search:\t{raw_acc}\n')
meta_file.write(f'Macro F1 after search:\t{macro_f1}\n')
meta_file.write(f'Precision after search:\t{precision}\n')
meta_file.write(f'Recall after search:\t{recall}\n')
