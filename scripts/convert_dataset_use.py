...
task_definition = (
    ...
    "Does this clause stipulate that the consumer is bound by the terms of use of a specific service, simply by using the service, without even being required to mark that he or she has read and accepted them?"
)
...
for _, row in df_train.iterrows():
    ...
    example = {
        ...
        "output": "Yes" if 'USE' in label_type_list else "No"  # Check if 'CR' is in the list
    }
    if 'USE' in label_type_list:
...
for _, row in df_test.iterrows():
    ...
    instance = {
        "input": row["text"],
        "output": ["Yes" if 'USE' in label_type_list else "No"]
    }
