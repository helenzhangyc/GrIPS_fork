...
task_definition = (
    ...
    "Does this clause describe an arbitration dispute resolution process that is not fully optional to the consumer?"
)
...
for _, row in df_train.iterrows():
    ...
    example = {
        ...
        "output": "Yes" if 'A' in label_type_list else "No"  # Check if 'A' is in the list
    }
    if 'A' in label_type_list:
...
for _, row in df_test.iterrows():
    ...
    instance = {
        "input": row["text"],
        "output": ["Yes" if 'A' in label_type_list else "No"]
    }
