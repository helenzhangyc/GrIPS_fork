...
task_definition = (
    ...
    "Does this clause stipulate that the service provider may suspend or terminate the service at any time for any or no reasons and/or without notice?"
)
...
for _, row in df_train.iterrows():
    ...
    example = {
        ...
        "output": "Yes" if 'TER' in label_type_list else "No"  # Check if 'CR' is in the list
    }
    if 'TER' in label_type_list:
...
for _, row in df_test.iterrows():
    ...
    instance = {
        "input": row["text"],
        "output": ["Yes" if 'TER' in label_type_list else "No"]
    }
