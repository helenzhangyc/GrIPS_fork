...
import os
import json
from expanded_encodeinstruction import *
...
def construct_instruction_prompt(mode, task_name, num_shots, num_test_instances, seed, null_word=None):
    if mode == "No Instructions" or mode == 0:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure=[
        ], number_of_instances=num_test_instances, seed=seed, null_word=null_word)
    elif mode == "Instruction Only" or mode == 1:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure=[
                                                                 "Definition"], number_of_examples=num_shots, number_of_instances=num_test_instances, seed=seed, null_word=null_word)
    elif mode == "Instruction + Examples" or mode == 2:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure=[
                                                                 "Definition", "Positive Examples Full Only"], number_of_examples=num_shots, number_of_instances=num_test_instances, seed=seed, null_word=null_word)
    else:
...
def complete_gpt3(prompt, l, model_name, temp=0.0, num_log_probs=None, echo=False, n=1):
    if "gpt-3.5" in model_name or "gpt-4" in model_name:
        ...
        # Use the original v1/completions for non-chat models
...
def complete_gpt2(prompt, l=10, model_name='gpt2-xl', num_log_probs=None, echo=False):
    ...
    if l > 0:
        ...
        total_sequences = model.generate(input_ids=input_ids['input_ids'].cuda(
        ), attention_mask=input_ids['attention_mask'].cuda(), max_length=l + len(input_ids['input_ids'][0]), do_sample=False)
    else:
    ...
    if num_log_probs is not None:
        ...
        logits = model.forward(input_ids=total_sequences, attention_mask=attention_mask,
                               position_ids=position_ids, return_dict=True).logits.detach().cpu()
        if not echo:
    ...
    for batch_id in range(len(prompt)):
        ...
        if not echo:
            curr_json['text'] = tokenizer.decode(
            ...
            if not echo:
                ...
                for current_element_top_log_probs, current_element_top_tokens in zip(top_log_probs[batch_id][:-1], top_tokens[batch_id][:-1]):
                    ...
                    curr_json['logprobs']['tokens'].append(
                        tokenizer.decode([current_element_top_tokens[0]]))
                ...
                for index in range(len(probs[batch_id])):
                    curr_json['logprobs']['tokens'].append(
                        tokenizer.decode([total_sequences[batch_id][index]]))
...
def complete(prompt, l, model_name='gpt-3.5-turbo', temp=0, num_log_probs=None, echo=False, n=None):
    ...
    if 'gpt2' in model_name:
        ...
        if key == 1:
            setup_gpt3('alternate_1_openai_key.txt')
        elif key == 2:
            setup_gpt3('alternate_2_openai_key.txt')
        else:
            setup_gpt3()
        return complete_gpt3(prompt, l=l, model_name=model_name, num_log_probs=num_log_probs, echo=echo, n=n)


def get_not_found_prob(prompt, label):
    prompt = prompt + label  # space already in label
    label_ans = complete(prompt, l=0, num_log_probs=1, echo=True)['choices'][0]
...
#                 label_probs[a, :, l] = np.exp(
#                     ans['logprobs']['token_logprobs'][0])
#             else:
#                 position = (a, l)
...
#         additional_prompt_batches, position_lookup = create_batches(
#             all_additional_prompts, all_missing_positions, batch_size=len(batch[0]))
#         for m, missing_batch in enumerate(additional_prompt_batches):
#             missing_response = complete(
#                 missing_batch, l=0, num_log_probs=1, echo=True)
#             for idx, missing_ans in enumerate(missing_response['choices']):
#                 which_sentence, which_label = position_lookup[m][idx]
#                 label_probs[which_sentence, :, which_label] = np.exp(
#                     missing_ans['logprobs']['token_logprobs'][-1])
#     assert (label_probs > 0).all(
#     ), "all should be populated with non-zero value"
...
def get_regular_label_probs(response, batch, labels, if_null=False):
    ...
    if isinstance(response, list):
        print("Plain list response:", response)
        label_probs = torch.zeros([len(response), 1, len(labels)])
        for i, ans in enumerate(response):
            for l, label in enumerate(labels):
                if label.strip() == ans.strip():  # Ensure exact match
                    label_probs[i, :, l] = 1  # Assign probability 1 for matches
        label_probs = label_probs / torch.sum(label_probs, dim=2, keepdim=True)
        return label_probs
    ...
    # Ensure the number of responses matches the batch size
    assert len(response['choices']) == len(batch), "Mismatch between batch size and API responses."
    ...
    for a, ans in enumerate(response['choices']):
        ...
        logprobs = ans.get('logprobs')
        if logprobs is None:
            raise ValueError("Logprobs not available in response. Make sure `logprobs` are requested in the API call.")
        
        for l, label in enumerate(labels):
            if label in logprobs['tokens']:
                label_probs[a, :, l] = np.exp(logprobs['token_logprobs'][logprobs['tokens'].index(label)])
            else:
                position = (a, l)
                all_missing_positions.append(position)
    ...
    if all_missing_positions:
        ...
        # Create additional prompt batches
        additional_prompt_batches, position_lookup = create_batches(
            all_additional_prompts, all_missing_positions, batch_size=len(batch)
        )
        ...
        for m, missing_batch in enumerate(additional_prompt_batches):
            ...
            for idx, missing_ans in enumerate(missing_response['choices']):
                which_sentence, which_label = position_lookup[m][idx]
                missing_logprobs = missing_ans.get('logprobs')
                if missing_logprobs is None:
                    raise ValueError("Logprobs not available in missing response.")
                
                label_probs[which_sentence, :, which_label] = np.exp(
                    missing_logprobs['token_logprobs'][-1]
                )
    
    # Ensure all probabilities are populated
    assert (label_probs > 0).all(), "All probabilities should be non-zero."
...
def get_null_label_probs(null_batch, labels):
    ...
    for l, label in enumerate(labels):
        ...
        label_response = complete(label_batch, l=0, num_log_probs=1, echo=True)
        for a, ans in enumerate(label_response['choices']):
            null_label_probs[a, :, l] = np.exp(
                ans['logprobs']['token_logprobs'][-1])
...
def run(mode, batch_size, num_shots, chosen_task_name, num_samples, seed=0, logit_only=False, override_prompts=False, function=None, split=None, task_labels=[], modified={}, if_calibrate=True):
    ...
    if not override_prompts:
        prompt_list, answer_list, index_list = construct_instruction_prompt(
            mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed)
    else:
        prompt_list, answer_list, index_list = function(
            mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, split=split, modified=modified)
        ...
        for nw in null_words:
            if not override_prompts:
                null_prompt_list, null_answer_list, null_index_list = construct_instruction_prompt(
                    mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, null_word=nw)
            else:
                null_prompt_list, null_answer_list, null_index_list = function(
                    mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, null_word=nw, split=split, modified=modified)
            assert index_list == null_index_list, pdb.set_trace()
            null_batches, _ = create_batches(
                null_prompt_list, null_answer_list, batch_size)
    ...
    for j in tqdm(range(len(all_batches))):
        batch = all_batches[j]
        responses = complete(batch, l=num_gen_tokens,
                             num_log_probs=num_top_tokens)
        label_probs = get_regular_label_probs(
            responses, batch, task_labels, if_null=logit_only)
        all_label_probs.append(label_probs)
        if logit_only:
            label_probs = label_probs / \
                torch.sum(label_probs, dim=2, keepdim=True)

        regular_preds = get_prediction(label_probs, task_labels)

        regular_accuracy_count += np.sum(
            evaluate_preds(regular_preds, batch_test_labels[j]))
...
# perform calibration, omitted in search as it is expensive.
        if if_calibrate:
            ...
            calibrated_probs = calibrated_probs / \
                torch.sum(calibrated_probs, dim=2, keepdim=True)
            if not logit_only:
                all_calibrated_probs.append(calibrated_probs)
            calibrated_preds = get_prediction(calibrated_probs, task_labels)

            calibrated_accuracy_count += np.sum(
                evaluate_preds(calibrated_preds, batch_test_labels[j]))
            all_calibrated_preds.extend([p.strip(" ")
                                        for p in calibrated_preds])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Take arguments from commandline')
    parser.add_argument('--mode', default="Prompt Only",
                        help='Type mode of instructions')
    parser.add_argument('--num-shots', default=2, type=int,
                        help='Type number of examples in the prompt if applicable')
    parser.add_argument('--batch-size', default=4, type=int,
                        help='Type in the batch-size')
    parser.add_argument('--task-idx', default=0, type=int,
                        help='Type in the batch-size')
    parser.add_argument('--seed', default=0, type=int,
                        help='Type in the batch-size')
    parser.add_argument('--model-name', default="curie",
                        help='Type name of gpt-3 model')
    ...
    file_contents = json.load(
        open("{}/{}".format(data_base_path, chosen_task_name)))
    num_samples = 100
    dest_path = "preds/{}/{}/".format(chosen_task, args.mode)
    if not os.path.exists(dest_path):
        os.makedirs(dest_path)

    label_probs, calibrated_probs, regular_accuracy_count, calibrated_accuracy_count, answer_list, index_list, task_labels = run(
        mode=mode, batch_size=batch_size, num_shots=num_shots, chosen_task_name=chosen_task_name, num_samples=num_samples, seed=args.seed, if_calibrate=False)
    print("Regular Accuracy:\t", np.round(
        100*regular_accuracy_count/len(answer_list), 2))
    assert len(all_regular_preds) == len(answer_list)
    assert all_answers == answer_list
    del all_answers
    print("Regular F1:\t", np.round(
        100*F1(answer_list, all_regular_preds, average='macro'), 2))
    if args.save_preds:
        results_lookup = [{'file': 'regular_predictions.txt', 'list': all_regular_preds}, {
            'file': 'calibrated_predictions.txt', 'list': all_calibrated_preds}, {'file': 'ground_truths.txt', 'list': answer_list}]
