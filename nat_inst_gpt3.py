...
def construct_instruction_prompt(mode, task_name, num_shots, num_test_instances, seed, null_word=None):
    if mode=="No Instructions" or mode==0:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure = [], number_of_instances = num_test_instances, seed=seed, null_word=null_word)
    elif mode=="Instruction Only" or mode==1:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure = ["Definition"], number_of_examples = num_shots, number_of_instances = num_test_instances, seed=seed, null_word=null_word)
    elif mode=="Instruction + Examples" or mode==2:
        prompt_list, answer_list, index_list = encodeinstruction(task_name, instruction_structure = ["Definition", "Positive Examples Full Only"], number_of_examples = num_shots, number_of_instances = num_test_instances, seed=seed, null_word=null_word)
    else:
...
def get_regular_label_probs(response, batch, labels, if_null=False):
    ...
    if len(all_missing_positions) > 0:
        ...
        for m, missing_batch in enumerate(additional_prompt_batches):
            missing_response = complete(missing_batch, l=0, num_log_probs=1, echo=True)
...
def run(mode, batch_size, num_shots, chosen_task_name, num_samples, seed=0, logit_only=False, override_prompts=False, function=None, split=None, task_labels = [], modified = {}, if_calibrate = True):
    ...
    if not override_prompts: prompt_list, answer_list, index_list = construct_instruction_prompt(mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed)
    else: prompt_list, answer_list, index_list = function(mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, split=split, modified=modified)
    ...
    if if_calibrate:
        for nw in null_words:
            if not override_prompts: null_prompt_list, null_answer_list, null_index_list = construct_instruction_prompt(mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, null_word=nw)
            else: null_prompt_list, null_answer_list, null_index_list = function(mode=mode, task_name=chosen_task_name, num_shots=num_shots, num_test_instances=num_samples, seed=seed, null_word=nw, split=split, modified=modified)
    ...
    for j in tqdm(range(len(all_batches))):
        batch = all_batches[j]
        responses = complete(batch, l=num_gen_tokens, num_log_probs=num_top_tokens)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', default=0, type=int, help='Type in the batch-size')
    ...
    label_probs, calibrated_probs, regular_accuracy_count, calibrated_accuracy_count, answer_list, index_list, task_labels = run(mode=mode, batch_size=batch_size, num_shots=num_shots, chosen_task_name=chosen_task_name, num_samples=num_samples, seed=args.seed, if_calibrate = False)
